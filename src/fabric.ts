...
export const fabricPluginId = "com.jonah791.agent-context"

export default function activate(ctx: FabricActivationContext): FabricActivation {
  ctx.log?.info("dsh-agent-context activated (Fabric skeleton)")
  // manifest 已声明以下 command（Host 可在插件运行前发现；此处只列 ID，SDK 发布后绑定 handler）：
  //   com.jonah791.agent-context.context — Show context occupancy and tokens spent for this session
